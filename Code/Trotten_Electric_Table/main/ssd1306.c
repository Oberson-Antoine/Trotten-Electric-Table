...
#include "esp_lcd_panel_io.h" // Add this include for esp_lcd_panel_io_tx_param
#include <string.h>
...
void set_pixel(int x, int y, bool on)
{
    ...
    if (on)
        ssd1306_framebuffer[byte_index] |= (1 << (y % 8));
    else
        ssd1306_framebuffer[byte_index] &= ~(1 << (y % 8));
}
...
void draw_char(int x, int y, char c)
{
    ...
    c = c & 0x7F;
    if (c < ' ')
    { // if unknown convert it to a space
        c = 0;
    }
    ...
    for (;;)
    {
        ...
        if (xQueueReceive(ssd1306_display_queue, &msg, portMAX_DELAY) == pdTRUE)
        {
            ...
            if (xSemaphoreTake(i2c_bus_mutex_handle, portMAX_DELAY) == pdTRUE)
            {
                ...
                case CONTRAST_SET:
                        ESP_ERROR_CHECK(esp_lcd_panel_io_tx_param(ssd1306_io_handle,0x81, &msg.contrast, 1));
                    break;
...
/**
 * Sets the screen contrast to the value
 * @param contrast : 0 to 256 value to set the contrast
 */
// void ssd1306_contrast_set(int8_t contrast)
// {
//     // uint8_t data[3] = {0x00, 0x81, contrast}; // 0x00 = control byte for command
//     uint8_t cmd[2] = {0x81, contrast}; // Command + value
//     SemaphoreHandle_t i2c_bus_mutex_handle = get_i2c_mutex();
//     if (xSemaphoreTake(i2c_bus_mutex_handle, portMAX_DELAY) == pdTRUE)
//     {
//         i2c_master_bus_handle_t i2c_bus_handle = get_i2c_bus(); // get the handle from the init

//         esp_lcd_panel_io_tx_param(ssd1306)
//             i2c_master_transmit(i2c_bus_handle, SSD1306_address, data, 3, 1000 / portTICK_PERIOD_MS);
//     }
// }