 ...
 * @CreatedDate           : 2025-06-14 18:51:58                              *
 * @LastEditors           : Oberson-Antoine<antoine.oberson@yahoo.fr>        *
 * @LastEditDate          : 2025-06-18 12:45:50                              *
 * @FilePath              : Trotten_Electric_Table/main/http_server.c        *
 ****************************************************************************/
...
// Embedded files : JQuery, index.html, app.css, app.js and favicon files
...
static void http_server_fw_update_reset_timer(void)
{
    ...
    {
        ESP_LOGI(TAG, "http_server_fw_update_reset_timer : firmware update successful");

        // give the webpage a chance to receive and acknoledge back and initialize the timer
 ...
 * @param req HTTTP request for which uri needs to be handled
 * @return ESP_OK
 */
 ...
 * @return ESP_OK, otherwise ESP_FAIL if timeout occurs and the update cannot be stated.
 */
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
    ...
    do
    {
        ...
        //is this the data we are receiving 
    ...
    // we won't update the global variable througout the file, so send the message about the status
 ...
 * and respond with the compile time/date when the page is first requested
 * @param req   http requeset for which the uri nedds to be handled
 * @return ESP_OK
 */
...
static httpd_handle_t http_server_configure(void)
{
    ...
    // adjust the task priority to 1 less than wifi application task
