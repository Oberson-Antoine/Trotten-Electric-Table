...
static void wifi_app_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
  ...
  /*
* Initialies the WiFi application event handler for Wifi and IP events
*/
...
static void wifi_app_soft_ap_config(void)
{
    ...
    memset(&ap_ip_info, 0x00, sizeof(ap_ip_info));  //set all the values to 0 in ap_ip_info

    esp_netif_dhcps_stop(esp_netif_ap);                         ///> must call this first
    inet_pton           (AF_INET, WIFI_AP_IP, &ap_ip_info.ip);  ///> assign access point's static IP, GW and netmask (converts the IP address string in WIFI_AP_IP (like "192.168.1.1") into its binary form and stores it in ap_ip_info.ip.)
    ...
    ESP_ERROR_CHECK(esp_netif_dhcps_start(esp_netif_ap));               ///> Start the AP DHCP server (for connecting e.g. your mobile phone)
    ...
    ESP_ERROR_CHECK(esp_wifi_set_bandwidth(ESP_IF_WIFI_AP, WIFI_AP_BANDWIDTH)); ///> default bandwidth 20MHz
    ESP_ERROR_CHECK(esp_wifi_set_ps(WIFI_STA_POWER_SAVE));              ///> powersave set to none
}
...
static void wifi_app_task(void *pvParameter)
{
    ...
    wifi_app_event_handler_init();
      ...
      // initialize thhe TCP/IP stack and wifi config
